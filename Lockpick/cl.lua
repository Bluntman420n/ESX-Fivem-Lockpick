...
local locked = false
RegisterNetEvent('Lockpick:startlockpicking')
AddEventHandler('Lockpick:startlockpicking', function()
    local ped = PlayerPedId()
    ...
    if distance < 3 then
        if lockstatus == 2 then
            TriggerEvent('Lockpick:client:openLockpick', lockpick)
...
function lockpick(success)
    ...
    if success then
        ...
        ClearPedTasks(PlayerPedId())
        ESX.ShowNotification('Picklocking Failed, Vehicle Alarms Went Off Run!')
        FreezeEntityPosition(PlayerPedId(), false)
